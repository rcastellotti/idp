...
We plan to fuzz the internal gRPC API of the dish to see if we manage to get access to internal statistics, additional information will be extracted by dissecting the companion Android application.
Eventually we will develop a toolset to interact with the API, in order to abstract away some of the logic with the end goal of making it easier to perform management operations. 
...
Starlink is an interesting project because it can provide Internet access to remote areas and could potentially be a revolution in how we connect to the Internet, a huge part of the reasons why this project is getting so popular is that it is able to provide a satellite-based low latency connection. Being able to measure performance of this new technology could be pivotal and we believe that our work could prove instrumental for further analysis.
\subsection*{Related Work}

There is not much literature about internals of the dish and about its API, there's an interesting hardware-level hacking talk given at DEFCON~\cite{defcon}, in which they managed to get a shell on the dish, however this hardware-level research won't be part of our research.
...
\subsection*{Planned Schedule}
  ...
  To complement my IDP, I chose \href{https://www.ce.cit.tum.de/en/lkn/lehre/vorlesungen/analysis-modeling-and-simulation-of-communication-networks}{"Analysis, Modeling and Simulation of Communication Networks"}. The IDP project work builds upon the foundations thaught on the couse. The covered topics (e.\,g., packet level simulation and topology models) will help to understand routing decisions potentially taken by the dish. Moreover, the practical approach to the arguments will be useful for the implementation of solutions to test performance and behaviour of the dish while using Starlink.
